...
exports.signUp = async (req, res) => {
  ...
  const check = await executeSQL(
    `SELECT user_id, user_email, user_password, username FROM users WHERE user_email = '${req.body.email}'`
  );
  ...
  try {
    ...
    const result = await executeSQL(
      `INSERT INTO users (user_id, user_email, user_password, username) 
      VALUES 
        (next value for dbo.seqUserIDs, 
            '${req.body.email}', 
            '${req.body.password}', 
            '${req.body.username}')`
    );
