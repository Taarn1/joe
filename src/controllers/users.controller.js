...
exports.findMatch = async (req, res) => {
  ...
  try {
    ...
    const check1 = await sqlHandler(
      `SELECT * FROM matches WHERE user1_id = ? AND user2_id = ?`,
      [Number(match[0].userid), Number(req.params.userid)]
    );
    const check2 = await sqlHandler(
      `SELECT * FROM matches WHERE user2_id = ? AND user1_id = ?`,
      [Number(match[0].userid), Number(req.params.userid)]
    );
    if (check1.length > 0 || check2.length > 0) {
      return res
        .status(400)
        .send("You have already matched with all compatible users");
    } else
      await sqlHandler(
...
exports.getMatches = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send("An error occurred while trying to get matches");
  }
};
